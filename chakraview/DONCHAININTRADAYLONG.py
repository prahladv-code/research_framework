...
class DONCHAININTRADAY(ChakraView):
    ...
    def check_dte_criteria(self, date: datetime.date, time: datetime.time):
        nearest_expiry = self.find_nearest_expiry_by_expiry_code(self.underlying, date, time, self.expiry_code)

        if nearest_expiry is None:
            logger.warning(f'NEAREST EXPIRY FOUND NONE AT: {date} {time}')
            return False
        
        date_difference = nearest_expiry - date
...
if __name__ == '__main__':
    dc = DONCHAININTRADAY()
    dc.run_backtest('DONCHAININTRADAYSHORT_NIFTY_15_15_1_0_0_2')
