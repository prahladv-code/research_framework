...
class ChakraView:
    ...
    def find_ticker_by_delta(self, underlying: str, date: datetime.date, time: datetime.time, delta: float, right: str, expiry_code: int):
        converted_delta = delta / 100

        tsdf = self.get_all_ticks_by_timestamp(underlying, expiry_code, date, time)

        if tsdf.empty:
            logger.warning(f"No Data Found For The Timestamp: {date} {time}")
            return {}

        filtered_df = tsdf[tsdf['right'] == right].copy()

        if filtered_df.empty:
            logger.warning(f"No '{right}' options found for {date} {time}")
            return {}

        # Drop NaNs safely
        filtered_df = filtered_df.dropna(subset=['Delta'])

        if filtered_df.empty:
            logger.warning(f"No valid Delta values for {date} {time}")
            return {}

        # Compute diff
        filtered_df['delta_diff'] = (filtered_df['Delta'].abs() - abs(converted_delta)).abs()

        if filtered_df['delta_diff'].isna().all():
            logger.warning(f"All delta_diff are NaN for {date} {time}")
            return {}

        # Safer than idxmin
        closest_row = filtered_df.nsmallest(1, 'delta_diff')

        return closest_row.iloc[0].to_dict()

    def find_ticker_by_strike(self, underlying: str, date: datetime.date, time: datetime.time, strike: int, right: str, expiry_code: int):
        timestampdf = self.get_all_ticks_by_timestamp(underlying, expiry_code, date, time)
        if not timestampdf.empty:
            filtered_df = timestampdf[(timestampdf['strike'] == strike) & (timestampdf['right'] == right)] 
            if filtered_df.empty:
                logger.warning(f'{strike} {right} Not Found For: {date} {time}')
                return {}
            return filtered_df.iloc[0].to_dict()
        else:
            logger.warning(f'No Data Found For: {date} {time}')
            return {}
