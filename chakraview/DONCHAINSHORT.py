...
class DONCHAIN(ChakraView):
    ...
    def gen_signals(self, row):
        ...
        # ENTRY
        if self.high_level is not None and self.low_level is not None:
            if self.in_position == 0:
                if row.c > self.high_level:
                    ...
                    if entry_ticker:
                        ...
                        self.target_price = entry_ticker['c'] * self.target_percentage
                        self.stoploss_level = self.low_level
                    ...
                    if entry_ticker:
                        ...
                        self.expiry = entry_ticker['expiry'].date()
                        self.target_price = entry_ticker['c'] * self.target_percentage
                        self.stoploss_level = self.high_level
                        entry_price = entry_ticker['c']
                        entry_trade = self.place_trade(current_timestamp, self.entry_symbol, entry_price, self.qty, self.qty * entry_price, 'SHORT', 'SHORT_ENTRY')
                        self.signals.append(entry_trade)
                        self.in_position = -1
            ...
            if self.in_position == 1:
                ...
                if position_tick:
                    ...
                    if price <= self.target_price:
                        logger.info(f"PUT TARGET EXIT FOUND AT : {current_timestamp}. SQUARING OFF.")
                    ...
                    logger.warning(f'PUT TARGET EXIT TICK FOUND EMPTY. RESETTING.')
                    self.reset_all_variables()
                ...
                if position_tick:
                    price = position_tick['c']
                    if price <= self.target_price:
                        logger.info(f"CALL TARGET EXIT FOUND AT : {current_timestamp}. SQUARING OFF.")
                        exit_trade = self.place_trade(current_timestamp, self.entry_symbol, price, self.qty, self.qty * price, 'COVER', 'SHORT_EXIT')
                        self.signals.append(exit_trade)
                        self.reset_all_variables()
                else:
                    logger.warning(f'CALL TARGET EXIT TICK FOUND EMPTY. RESETTING.')
                    self.reset_all_variables()
